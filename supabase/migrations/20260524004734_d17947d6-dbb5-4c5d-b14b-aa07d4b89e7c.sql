-- Single Ments no longer have a timer; the RPC is no longer used.
DROP FUNCTION IF EXISTS public.extend_single_ment_timer(uuid);

-- Chain Pause Token now adds 48 hours on top of remaining time (was 24).
...
BEGIN
  ...
  UPDATE ment_chains
  SET expires_at = GREATEST(expires_at, now()) + INTERVAL '48 hours'
