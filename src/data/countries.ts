// Comprehensive country dial-code list for phone verification.
// `dial` is the display string (may contain a dash, e.g. "+1-242").
// Use `dialPrefix()` to get the E.164 prefix to prepend (dashes removed).

export interface Country {
  name: string;
  iso2: string; // ISO 3166-1 alpha-2, used to derive the flag emoji
  dial: string; // display dial code
}

// Convert an ISO alpha-2 code into its flag emoji (regional indicator symbols).
...
// The E.164 prefix to prepend to the local number (dashes/spaces stripped).
export function dialPrefix(dial: string): string {
  return dial.replace(/[^\d+]/g, '');
}

// US / Canada / UK pinned to the top; everything else alphabetical by name.
export const COUNTRIES: Country[] = [
  ...
  { name: 'Dominican Republic', iso2: 'DO', dial: '+1-809' },
  ...
  { name: 'Puerto Rico', iso2: 'PR', dial: '+1-787' },
